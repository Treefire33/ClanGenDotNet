...
namespace ClanGenDotNet.Scripts.Game_Structure
{
	public class Game(string currentScreen = "start screen")
	{
		...
		public void SwitchSetting(string settingName)
		{
			...
			if (listIndex == SettingsLists[settingName].Count - 1)
			{
				Settings[settingName] = SettingsLists[settingName][0];
			}
			else
			{
				Settings[settingName] = SettingsLists[settingName][listIndex + 1];
			}
		}
		...
		public void SetSettingsFromLoaded()
		{
			...
			{
				var setting = game.GameSettings.General[key];
		...
		public void SaveSettings()
		{
			...
			try
			{
				SafeSave(DataDirectory.GetSaveDirectory() + "\\settings.json", this.Settings);
			}
			catch
			{
		...
		{
			string data;
			if (writeData.GetType() != typeof(string))
			{
				data = JsonConvert.SerializeObject(writeData);
			}
			else
			{
			...
			if (checkIntegrity)
			{
				...
				while (true)
				{
					using var writeFile = new StreamWriter(tempFilePath);
					writeFile.Write(data);
					writeFile.Flush();
					writeFile.BaseStream.Flush();

					using var openFile = new StreamReader(tempFilePath);
					var readData = openFile.ReadToEnd();
